...
## Modelo de Predicción Basado Únicamente en Viento

```{r}
# 1. PREPARACIÓN DE DATOS PARA PREDICCIÓN A 1 HORA
...
d_pred <- datos %>%
  ...
  mutate(
    ...
    diferencia_horas = as.numeric(
      difftime(fecha, lag(fecha), units = "hours")
    ),
    
    # Variables de la hora anterior
    SO2_lag1   = lag(SO2),
    WDR_lag1   = lag(WDR),
    WSR_lag1   = lag(WSR),
    TOUT_lag1  = lag(TOUT),
    ...
    PRS_lag1   = lag(PRS),
    SR_lag1    = lag(SR),
    RAINF_lag1 = lag(RAINF)
  ) %>%
  ungroup() %>%
  
  # Conservar únicamente casos donde la observación anterior
  # corresponde exactamente a una hora antes
  filter(diferencia_horas == 1) %>%
  
  # Dirección del viento de t-1 como variable circular
  mutate(
    WDR_sin_lag1 = sin(2 * pi * WDR_lag1 / 360),
    WDR_cos_lag1 = cos(2 * pi * WDR_lag1 / 360)
  )
```

### Crear una muestra común y separar entrenamiento/prueba

```{r}
# 2. MUESTRA COMÚN Y DIVISIÓN TEMPORAL

d_modelos <- d_pred %>%
  select(
    fecha, Estacion, episodio,
    SO2_lag1,
    WDR_sin_lag1, WDR_cos_lag1, WSR_lag1,
    TOUT_lag1, RH_lag1, PRS_lag1,
    SR_lag1, RAINF_lag1
  ) %>%
  drop_na() %>%
  mutate(
...
# Prueba real fuera de muestra: 2025
test <- d_modelos %>%
  filter(anio == 2025)

cat("Entrenamiento:", nrow(train), "observaciones\n")
cat("Prueba 2025:", nrow(test), "observaciones\n")
```
...
# Modelo 0: únicamente estado previo del SO2
mod_base <- glm(
  episodio ~ SO2_lag1,
  data = train,
  family = binomial
...
# Modelo 2: SO2 anterior + meteorología completa
mod_meteo <- glm(
  episodio ~
    SO2_lag1 +
    WDR_sin_lag1 +
...
```{r}
# ============================================================
# 4. PREDICCIÓN FUERA DE MUESTRA - 2025
# ============================================================
...
```{r}
# ============================================================
# 5. COMPARACIÓN DE DESEMPEÑO PREDICTIVO
# ============================================================
...
roc_viento <- roc(
  test_pred$episodio,
  test_pred$prob_viento,
  quiet = TRUE
)

roc_meteo <- roc(
  test_pred$episodio,
  test_pred$prob_meteo,
  quiet = TRUE
)

# AUC
auc_base   <- as.numeric(auc(roc_base))
auc_viento <- as.numeric(auc(roc_viento))
auc_meteo  <- as.numeric(auc(roc_meteo))

# Brier Score
brier_base <- mean(
  (test_pred$episodio - test_pred$prob_base)^2
)

brier_viento <- mean(
  (test_pred$episodio - test_pred$prob_viento)^2
)

brier_meteo <- mean(
  (test_pred$episodio - test_pred$prob_meteo)^2
)
...
```{r}
# ============================================================
# 6. CURVAS ROC DE LOS TRES MODELOS
# ============================================================

plot(
  roc_base,
  legacy.axes = TRUE,
  main = "Capacidad predictiva a una hora"
)
...
legend(
  "bottomright",
  legend = c(
    paste0("SO2 anterior: AUC = ", round(auc_base, 3)),
    paste0("SO2 + viento: AUC = ", round(auc_viento, 3)),
    paste0("SO2 + meteorología: AUC = ", round(auc_meteo, 3))
  ),
  lty = 1
)
```
