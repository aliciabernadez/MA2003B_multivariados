...
# 1. Función para calcular la Moda
...
# 3. Distribución de Frecuencia para Variables Cualitativas
frecuencia_estaciones <- datos %>% count(Estacion) %>% mutate(Porcentaje = n / sum(n) * 100)
frecuencia_anios <- datos %>% count(Año) %>% mutate(Porcentaje = n / sum(n) * 100)
```
=======
>>>>>>> 53a54ce990870e51ed65d5e43242ca8504468ee6
