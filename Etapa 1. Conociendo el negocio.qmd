...
datos_transformados <- datos_imputados %>%
  mutate(
    #Transformación para variables cíclicas como el viento ya que va de 0 a 360°
    WDR_sin = sin(2 * pi * WDR / 360),
    WDR_cos = cos(2 * pi * WDR / 360),
    
    #Transformación logarítmica para suavizar picos extremos en los niveles de azufre
    across(c(SO2), ~log1p(.)),
    
    #Estandarizar al valor z todas las variables
    across(c(SO2, RH, WSR, TOUT, SR, PRS, RAINF), 
           ~as.numeric(scale(.)))
  ) %>%
