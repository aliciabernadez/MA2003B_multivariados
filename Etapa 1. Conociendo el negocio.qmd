...
Se puede notar que hay una gran cantidad de datos faltantes. Especialmente en la columna de TOUT donde hubo 13,408 valores faltantes. Otras variables con gran cantidad de datos faltantes son $RH$, $SO_2$ y RH.
...
#Imputación con mediana por Estación, año, mes y hora
datos_imputados <- datosF %>%
  group_by(Estacion, Año, Mes, Hora) %>%
  mutate(across(where(is.numeric), ~ifelse(is.na(.), median(., na.rm = TRUE), .))) %>%
  ungroup()

#Imputación sin tomar en cuenta el año para los datos que todavia quedaron faltantes en los que no habia datos para todo un mes durante un año
datos_imputados <- datos_imputados %>%
  group_by(Estacion, Mes, Hora) %>%
  mutate(across(where(is.numeric), ~ifelse(is.na(.), median(., na.rm = TRUE), .))) %>%
  ungroup()

#Imputación para los pocos datos faltantes que quedaron ahora sin tomar en cuenta la estación, solo el mes y la hora
datos_imputados <- datos_imputados %>%
  mutate(
    Mes = month(`Fecha y hora`),
    Hora = hour(`Fecha y hora`)
