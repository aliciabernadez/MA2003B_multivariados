...
Los antecedentes revisados coinciden en que el análisis conjunto de concentraciones y variables meteorológicas permite identificar patrones compatibles con fuentes de emisión específicas. Ninguno aborda, sin embargo, una cuestión central para la gestión de la calidad del aire: si los indicadores estadísticos empleados habitualmente en los instrumentos de planeación representan de manera adecuada la exposición que experimenta la población cercana a una fuente puntual. Este proyecto aplica el enfoque direccional a los registros del Sistema Integral de Monitoreo Ambiental y lo extiende en esa dirección, al cuantificar la magnitud de la exposición aguda vinculada al sector identificado y contrastarla con lo que reportarían las medidas de tendencia central. El resultado permite evaluar si los mecanismos de seguimiento vigentes capturan el fenómeno que buscan vigilar.

# Problemática y objetivo

## Planteamiento

El Área Metropolitana de Monterrey concentra una parte considerable de la actividad industrial del noreste del país, y su configuración geográfica, parcialmente delimitada por elevaciones de la Sierra Madre Oriental, genera una cuenca atmosférica semicerrada donde la dispersión depende de manera crítica del régimen de vientos. Dentro de ese contexto, el dióxido de azufre presenta un comportamiento distinto al de los demás contaminantes criterio, ya que su origen se asocia principalmente a la combustión de hidrocarburos con contenido de azufre, característica de procesos industriales específicos más que del tráfico vehicular distribuido. Esta particularidad lo convierte en un trazador útil para el estudio de fuentes puntuales.

El problema que motiva el trabajo es metodológico antes que descriptivo. Las concentraciones promedio permiten caracterizar el comportamiento general del contaminante, pero pueden ocultar episodios cortos de concentraciones elevadas. Si la contribución de una fuente industrial se manifiesta en forma de eventos agudos de pocas horas, un sistema de seguimiento basado en promedios anuales podría reportar condiciones estables mientras la exposición real de la población cercana se deteriora. Conviene precisar que el objetivo del proyecto se reformuló durante su desarrollo. En las primeras etapas se planteó como una pregunta de identificación, orientada a determinar si existía un patrón direccional consistente con la influencia de una fuente industrial puntual. Los resultados de la etapa de modelado permitieron considerar esa pregunta resuelta, por lo que el énfasis se desplazó desde la atribución hacia la cuantificación de la exposición y la evaluación de los indicadores empleados para vigilarla.
...
Las técnicas anteriores se organizaron en una secuencia de seis pasos en la que cada uno responde a una explicación alternativa distinta: identificación direccional sin imponer el rumbo relevante, comparación espacial entre las cuatro estaciones, control por variables meteorológicas, control por la memoria temporal del propio contaminante, modelo integrado que enfrenta ambas explicaciones sobre las mismas observaciones, y verificaciones de robustez. La correspondencia detallada entre cada pregunta de validación, el método empleado y la explicación que permite descartar se presenta en el Anexo F.
...
Los modelos se sometieron a cinco verificaciones independientes. La validación fuera de muestra estimó los modelos con 2022 a 2024 y los evaluó sobre 2025, año que el ajuste nunca observó, reportando el área bajo la curva ROC como medida de discriminación y el error de Brier como medida de calibración. La inferencia robusta recalculó los errores estándar agrupando por día, para corregir la subestimación de la incertidumbre que produciría suponer independencia entre horas de una misma jornada. El análisis de sensibilidad replicó la estimación con los percentiles 90, 95 y 99. El diagnóstico de autocorrelación residual evalúa si la especificación omite estructura temporal relevante. Finalmente, la evaluación de calibración compara por deciles de riesgo la probabilidad predicha frente a la proporción observada, verificación pertinente porque un modelo puede discriminar correctamente y aun así producir probabilidades sesgadas.

## Especificaciones exploradas que no resultaron viables

Tres aproximaciones se descartaron durante el desarrollo. La regresión lineal sobre la concentración se abandonó por las razones expuestas al inicio de esta sección, y sus resultados se incluyen en el Anexo A. La partición en dos conglomerados, sugerida por el índice de silueta, resultó menos informativa que la de cuatro grupos, decisión que se detalla en el Anexo B. La estimación del modelo direccional en la estación de control con el umbral más exigente no fue posible, dado que registra menos de treinta horas por encima de 30.6 partes por billón en cuatro años, imposibilidad que constituye en sí misma un resultado.
...
La aplicación del mismo procedimiento a SE2 y NE, incluida en el Anexo E, confirma el gradiente obtenido por vía paramétrica: ninguna de las dos estaciones presenta un lóbulo orientado hacia el azimut de la fuente, y sus probabilidades condicionales máximas son uno y dos órdenes de magnitud menores, respectivamente.
