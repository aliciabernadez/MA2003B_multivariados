...
El Área Metropolitana de Monterrey concentra una importante actividad industrial y presenta condiciones geográficas que hacen que la dispersión de contaminantes dependa del régimen de vientos. En este contexto, el dióxido de azufre resulta especialmente relevante, ya que se relaciona principalmente con procesos de combustión de combustibles con contenido de azufre y puede actuar como indicador de fuentes industriales puntuales. El problema central es que los indicadores de tendencia central pueden ocultar episodios breves de concentraciones elevadas. Si la contribución de una fuente se manifiesta principalmente mediante eventos de pocas horas, los promedios o medianas pueden mostrar condiciones relativamente estables aun cuando la frecuencia de exposición aguda sea considerable.
...
SE3 es la estación más cercana al complejo refinador y presenta un azimut de 87.3°, correspondiente al Este, mientras que SE2 y NE permiten evaluar la atenuación de la señal conforme aumenta la distancia. GAR, ubicada a 68.5 km, se utiliza como estación de control. Las cuatro estaciones tienen el complejo hacia el Este o Este-Sureste. Por ello, si la asociación con este sector se debiera únicamente a un fenómeno meteorológico regional, se esperaría un comportamiento similar en todas. La ubicación aproximada del centroide no modifica esta clasificación: al desplazarlo 1.5 km en cada dirección, el azimut desde SE3 permanece dentro del rango ENE-ESE.

# Preparación de la base de datos

## Origen y estructura

La información procede de los registros horarios de SIMA correspondientes a 2022-2025 y del catálogo de rangos de validación proporcionado por el socio formador. La base final integra 132,936 observaciones de las cuatro estaciones seleccionadas e incluye ocho contaminantes, así como dirección y velocidad del viento, temperatura, humedad relativa, presión, radiación solar y precipitación. SO2 constituye la variable principal de interés; los demás contaminantes se emplearon como trazadores de contraste y las variables meteorológicas como controles.

## Validación y tratamiento de datos faltantes

Los registros fuera de los rangos de validación proporcionados por SIMA se transformaron en valores faltantes. Debido a cambios en la instrumentación, estos límites varían según el año, por lo que la validación se realizó considerando simultáneamente año y variable. Las variables meteorológicas de control se imputaron mediante vecinos más cercanos, mientras que los contaminantes conservaron sus valores faltantes para evitar introducir artificialmente patrones químicos. SO2 tampoco se depuró mediante criterios automáticos de valores extremos, ya que las concentraciones elevadas constituyen precisamente el fenómeno de interés.
...
La base final constituye un panel no balanceado, ya que las cuatro estaciones no cuentan con exactamente el mismo número de observaciones válidas durante el periodo analizado. La disponibilidad de datos varía entre 32,534 horas en GAR y 33,977 horas en SE3, diferencias que responden principalmente a la disponibilidad efectiva de los sensores y a los registros descartados durante el proceso de validación. No se realizó ningún procedimiento para igualar artificialmente el número de observaciones entre estaciones, ya que esto habría implicado eliminar información válida. Por esta razón, los análisis se realizaron utilizando todas las mediciones disponibles para cada estación, manteniendo la estructura real de la base de datos.

# Modelación y validación

## Criterio de selección de técnicas

La selección de técnicas respondió a tres características principales del problema: la variable de interés corresponde a la ocurrencia o no de episodios elevados, SO2 presenta una distribución fuertemente asimétrica y la dirección del viento es una variable circular. Por estas razones se utilizó regresión logística para modelar directamente la probabilidad de episodio, evitando depender de supuestos de normalidad sobre la concentración. De forma complementaria, se aplicaron componentes principales y k-means para estudiar la estructura conjunta de los contaminantes sin utilizar información meteorológica, permitiendo contrastar si SO2 sigue un comportamiento químico similar o distinto al de los principales trazadores urbanos.
...
La frecuencia de episodios disminuye de manera marcada entre estaciones. SE3 registra 11.63% de horas de episodio frente a 0.15% en GAR, una diferencia cercana a 77 veces. En contraste, las medianas se mantienen entre 3.3 y 4.4 ppb, mientras que la cola superior muestra diferencias mucho mayores. Esto indica que el principal contraste entre sitios se encuentra en la frecuencia y severidad de los eventos agudos, no en sus niveles típicos.
...
El Este presentó la mayor asociación con los episodios (OR = 6.12; IC 95%: 4.73-7.92), seguido por ESE y ENE, mientras que los rumbos occidentales mostraron razones de momios inferiores a uno. Este patrón resulta opuesto al esperado si los episodios provinieran principalmente del núcleo urbano localizado al oeste de Cadereyta. El modelo fue significativo frente al nulo (Δ devianza = 4,617.2; gl = 22) y alcanzó AUC = 0.811. El rumbo de mayor asociación coincide además con el azimut aproximado del complejo desde SE3, cercano a 87°.
...
La asociación con el sector Este se atenúa conforme aumenta la distancia: la razón de momios disminuye de 3.19 en SE3 a 1.58 en SE2 y deja de ser significativa en NE y GAR. Este patrón sugiere que la señal depende simultáneamente de la dirección y la proximidad, en lugar de responder a un efecto meteorológico regional común a todas las estaciones.
...
La diferencia entre estaciones se confirmó mediante un modelo conjunto con interacción entre estación y sector (Δ devianza = 195.77; gl = 3), favorecido también por los criterios AIC y BIC. En conjunto, los resultados son consistentes con una señal que pierde intensidad conforme aumenta la distancia.

## Persistencia del patrón ante controles meteorológicos

Para evaluar si la asociación direccional podía explicarse por otras condiciones meteorológicas, el modelo se amplió con temperatura, humedad relativa, presión atmosférica, radiación solar y precipitación mediante splines naturales. La selección por BIC descartó la precipitación y mantuvo las variables restantes. Aun con estos controles, el bloque direccional conservó una contribución significativa (Δ devianza = 1,192.7; gl = 15; p \< 0.001).
...
La ampliación meteorológica apenas modificó el desempeño fuera de muestra: el AUC pasó de 0.789 a 0.794 y el error de Brier de 0.104 a 0.106. Por tanto, estas variables funcionan como controles, pero no sustituyen la información aportada por la dirección del viento.

## Persistencia del patrón ante la memoria temporal del contaminante

La autocorrelación residual mostró que la dependencia temporal seguía presente: a una hora fue de 0.484 en el modelo base y de 0.471 después de incorporar controles meteorológicos. Por ello, se añadieron rezagos de SO2 de 1, 2, 12 y 24 horas, seleccionados mediante BIC.

Los rezagos mejoraron significativamente el ajuste (Δ devianza = 5,187.7; gl = 4), pero el sector oriental mantuvo una razón de momios de 3.34 (IC 95%: 3.03-3.68). Esto indica que la asociación direccional no se explica únicamente por la persistencia temporal del contaminante.
...
La prueba central del análisis consiste en eliminar el sector direccional del modelo integrado, conservando simultáneamente la meteorología ampliada y los rezagos. La devianza se deteriora en 600.2 puntos sobre un grado de libertad, con un valor p \< 0.001. Esto confirma que el sector oriental aporta información adicional aun después de controlar por meteorología y dependencia temporal.
...
SO2 fue el único contaminante con una asociación significativamente positiva con el sector Este (OR = 2.66). CO y O3 no mostraron asociación significativa, mientras que NO y NOX presentaron razones de momios cercanas a 0.22. Este comportamiento opuesto al de los principales trazadores vehiculares es consistente con un perfil distinto al de la combustión urbana difusa.

## Verificación por método no paramétrico

```{r}
#| label: fig-cbpf-contraste
#| fig-cap: "Función de probabilidad bivariada condicional en SE3: SO2 (izquierda) y NOx (derecha)."
#| fig-subcap:
#|   - "Dióxido de azufre"
#|   - "Óxidos de nitrógeno"
#| layout-ncol: 2
...
knitr::include_graphics(c(
  "figuras/cbpf_so2_se3.png",
  "figuras/cbpf_nox_se3.png"
))
```

La CBPF confirmó el contraste direccional: SO2 presenta un lóbulo definido hacia el Este y velocidades moderadas, mientras que NOX alcanza su máximo en condiciones de calma y sin orientación preferente. La coincidencia entre este método no paramétrico y la regresión logística refuerza la robustez del patrón. Los resultados para SE2 y NE se presentan en el Anexo E.
...
El primer componente explica 48.6% de la variabilidad y está dominado por los principales trazadores de combustión urbana, mientras que SO2 presenta una carga prácticamente nula de 0.029. En contraste, en el segundo componente, que explica 19.4%, SO2 presenta la mayor carga (0.604). Esto indica que las horas con aumentos de SO2 siguen una estructura de variación distinta a la de los principales trazadores urbanos.
...
Los conglomerados muestran perfiles químicos claramente diferenciados. El grupo 1 concentra 72.2% de los episodios y presenta SO2 elevado (16.2 ppb), NOX bajo (10.4 ppb) y 50.5% de horas con viento del Este. El grupo 2 presenta el comportamiento opuesto, con SO2 de 4.4 ppb, NOX de 51.8 ppb y solo 6.2% de horas del Este. La razón SO2/NOX es 1.56 frente a 0.085, respectivamente, reforzando la existencia de un perfil de SO2 distinto al de la acumulación urbana.
...
Las diferencias de medianas entre SE3 y GAR son estadísticamente significativas, pero pequeñas en términos absolutos: varían entre 0.6 y 1.5 ppb y disminuyen hasta su mínimo en 2025. Sin embargo, la frecuencia de episodios muestra el comportamiento opuesto: SE3 registró 689 horas en 2023, cuando la diferencia de medianas fue máxima, y 1,172 en 2025, cuando fue mínima. Esto evidencia que la tendencia central describe el nivel habitual, pero representa de forma limitada los cambios en la cola superior de la distribución.
...
En conjunto, los resultados muestran que los episodios elevados de SO2 en SE3 presentan una asociación específica con el sector Este, cuya intensidad disminuye con la distancia y persiste después de controlar por meteorología y dependencia temporal. La ausencia de un patrón equivalente en los principales trazadores urbanos, junto con los resultados de PCA, conglomerados y CBPF, respalda la existencia de un perfil químico y direccional diferenciado. El principal hallazgo práctico es que esta señal se manifiesta en la frecuencia de eventos agudos: mientras las medianas entre estaciones son similares, SE3 registra un exceso aproximado de 976 horas de episodio al año respecto de GAR.

## Alcance de las conclusiones

Debido al carácter observacional del estudio, los resultados no permiten atribuir causalidad directa a una instalación específica. Sin embargo, la combinación del gradiente espacial, los controles meteorológicos y temporales, la especificidad química y la concordancia entre métodos independientes reduce la plausibilidad de las explicaciones alternativas evaluadas. Persisten como limitaciones la ausencia de un inventario georreferenciado de emisiones y el uso de coordenadas aproximadas del complejo.

## Implicaciones para el seguimiento de la calidad del aire

El contraste entre tendencia central y exposición aguda tiene consecuencias operativas. En 2025, las medianas sugieren convergencia entre SE3 y GAR, aunque la frecuencia de episodios aumentó. Además, dado que los eventos duran dos horas en mediana, el promedio móvil de 24 horas reduce el percentil 99 de 55.3 a 26.0 ppb. Por ello, las métricas agregadas describen adecuadamente el comportamiento general, pero deben complementarse con indicadores sensibles a eventos breves, especialmente para un contaminante asociado con exposiciones de corta duración (Secretaría de Medio Ambiente del Estado de Nuevo León, 2024).

## Recomendaciones

Del análisis se derivan cuatro recomendaciones para el socio formador. Primero, incorporar métricas de frecuencia de excedencia como complemento de los promedios anuales, utilizando la información que la red ya genera (Secretaría de Medio Ambiente del Estado de Nuevo León, 2024). Segundo, evaluar un esquema de alerta horaria condicionado a la dirección del viento en SE3, considerando que el modelo integrado alcanzó AUC = 0.896 fuera de muestra. Tercero, utilizar el exceso de horas de episodio respecto de una estación de control como indicador complementario de exposición aguda. Finalmente, reforzar el monitoreo entre Cadereyta y Juárez para caracterizar con mayor detalle la atenuación espacial de la señal.

## Preguntas que surgen del análisis

Los resultados abren varias líneas de investigación: identificar las causas del repunte de episodios observado en 2025; evaluar la estabilidad temporal del patrón direccional; comparar periodos de operación normal y mantenimiento del complejo; estimar la población potencialmente expuesta; e incorporar un inventario georreferenciado de emisiones para distinguir entre posibles fuentes ubicadas en el mismo sector oriental.

## Conclusión

Los resultados son consistentes con la influencia de una fuente puntual de SO2 ubicada al Este de SE3, cuya señal disminuye con la distancia y presenta una firma química distinta a la de la combustión urbana. La coincidencia entre el sector identificado, la ubicación del complejo refinador y los análisis de falsación hace de esta fuente la explicación más parsimoniosa del patrón observado.

El hallazgo de mayor relevancia práctica es que las medidas de tendencia central representan de forma limitada la exposición aguda: una diferencia de medianas inferior a 1 ppb puede coexistir con un exceso aproximado de 976 horas anuales de episodio. Por ello, la frecuencia de eventos elevados aporta información que no se observa únicamente mediante medias o medianas.
...
Se utilizó inteligencia artificial generativa como herramienta de apoyo durante el desarrollo del presente proyecto.

**1. Herramienta utilizada:** ChatGPT

**2. Uso realizado:** La herramienta se utilizó como apoyo para la generación, revisión y depuración de código en R; propuesta de alternativas de modelación y validación; organización de resultados; y revisión de redacción, claridad y estructura del documento. La IA también se utilizó para apoyar la búsqueda de errores, comparar posibles enfoques metodológicos y facilitar la interpretación de resultados obtenidos por el equipo.

**3. Secciones donde se utilizó:** Principalmente en la preparación y tratamiento de los datos, modelación y validación estadística, interpretación de resultados, elaboración de gráficos y tablas mediante código, discusión, conclusiones y revisión de la redacción general del reporte y sus anexos.

**4. Validación realizada por los estudiantes:** Todo el código utilizado fue ejecutado por los integrantes del equipo sobre las bases de datos del proyecto. Los resultados numéricos, modelos, gráficos y pruebas estadísticas fueron revisados y contrastados con las salidas obtenidas directamente en R. Asimismo, el equipo verificó la congruencia entre los resultados obtenidos, su interpretación estadística y las conclusiones presentadas. Las decisiones metodológicas finales fueron tomadas por los integrantes del equipo.

**5. Responsabilidad sobre el contenido:** Los integrantes del equipo revisaron y verificaron el contenido generado o sugerido mediante inteligencia artificial antes de incorporarlo al documento. El equipo asume la responsabilidad académica por el análisis, las decisiones metodológicas, las interpretaciones, las fuentes utilizadas y el contenido final entregado.

**6. Otro:** La inteligencia artificial se utilizó únicamente como herramienta de apoyo y no sustituyó la ejecución de los análisis, la revisión de los resultados ni el criterio académico de los integrantes del equipo. No se consideraron como resultados válidos afirmaciones, cálculos o referencias generadas por IA que no pudieran ser verificadas mediante los datos, el código ejecutado o las fuentes consultadas.
