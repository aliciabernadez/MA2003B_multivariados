...
- **Técnica de dependencia: regresión logística binaria:** Se eligió regresión logística en lugar de lineal para no diluir los picos extremos en el promedio y manejar de mejor manera la asimetría del $SO_2$. Como trabaja directamente la probabilidad de sufrir un episodio agudo controlando la velocidad del viento (vía splines) y la temporalidad de forma cíclica, nos ayuda a aislar el efecto de la dirección del viento sin sesgos geográficos previos.

<!-- -->

- **Técnicas de interdependencia: componentes principales y conglomerados:** Utilizados para analizar la estructura química del aire por sus contaminantes. Su objetivo es comprobar que el comportamiento del $SO_2$ es independiente a los gases vehiculares ($NO_X$, $CO$), lo que confirma la hipótesis de una fuente de origen industrial.

- **Técnica complementaria: contraste no paramétrico de medianas:** Se empleó la prueba de Kruskal-Wallis y el estimador de desplazamiento de Hodges-Lehmann para cuantificar diferencias típicas entre estaciones sin asumir normalidad. Esto permitió contrastar explícitamente la magnitud de las variaciones de tendencia central frente a la frecuencia de eventos agudos, fundamentando el argumento central para la política pública.
...
La aplicación de la CBPF a las tres estaciones con señal confirma que el efecto se va atenuando conforme se avanza a las demás estaciones.
...
Como podemos ver en la Tabla, el grupo 1, que incluye el 14.7% de los datos, concentra el 72.2% de los episodios de $SO_2$ con viento predominantemente del este. Por el contrario, el grupo 2 representa acumulación urbana, con alto $NO_X$ y viento en calma. La razón $SO_2$/$NO_X$ del grupo 1 es 18 veces mayor que la del grupo 2, confirmando estadísticamente la firma química de azufre independiente de la combustión vehicular.
...
```{r fig3, fig.width=6.5, fig.height=3.8, fig.cap="Evolución anual de la diferencia de medianas entre SE3 y GAR (línea, eje derecho) frente al número de horas de episodio en SE3 (barras, eje izquierdo). Ambas series evolucionan en sentidos opuestos."}
comp <- hl %>%
  select(Año, Dif_HL) %>%
...
El modelo logístico alcanzó un buen ajuste con un valor Pseudo-R2=0.189 con un AOC de 0.783 y 0.764 dentro y fuera de la muestra respectivamente, lo cual descarta un sobreajuste. El error de Brier de 0.1065 mejoró la tasa base teniendo sensibilidad de 0.712 y especificidad de 0.681. El PCA presentó un KMO de 0.623 y prueba de Bartlett significativa.
...
Los resultados obtenidos son consistentes con la existencia de una fuente puntual de \$SO\_{2}\$ ubicada al este de la estación SE3, cuya influencia se atenúa con la distancia y cuya firma química difiere de la de fuentes de combustión urbana. Esta hipótesis se sustenta de lo siguiente:

Primero, la regresión logística identificó independientemente que los vientos del este incrementan la probabilidad de episodios agudos, lo cual coincide con el azimut geográfico de la refinería. Además, este efecto decae con la distancia, característico de una pluma de dispersión.

También las técnicas de interdependencia demostraron que el $SO_2$ es ortogonal a los gases vehiculares. Con vientos del este, la probabilidad de altos niveles de $SO_2$ sube, mientras que la de $NO_X$ se reduce drásticamente, descartando al tráfico como fuente.

Fianlmente, él fenómeno de contaminación impacta la frecuencia de eventos extremos y no el nivel de fondo. Mientras tanto, las medianas anuales entre estaciones casi no difieren y Cadereyta sufre de 976 horas anuales de exposición adicionales frente a la estación de control.
